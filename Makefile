# Makefile for Exact Online SDK development tasks
...
help: ## Display this help message
	@echo "Exact Online SDK Development Tasks"
	@echo "===================================="
...
run-mypy: ## Run mypy using uv with proper configuration
	MYPYPATH=$(SRC_DIR) $(UV) run mypy -p exact_online_sdk
...
test-coverage: ## Run tests with coverage report using uv
	$(UV) run pytest $(TEST_DIR) --cov=$(SRC_DIR) --cov-report=term-missing --cov-report=html --cov-report=xml --cov-fail-under=100 -v

.PHONY: test-coverage-html
test-coverage-html: ## Run tests with HTML coverage report using uv
	$(UV) run pytest $(TEST_DIR) --cov=$(SRC_DIR) --cov-report=html -v
	@echo "HTML coverage report generated in htmlcov/index.html"

.PHONY: test-coverage-xml
test-coverage-xml: ## Run tests with XML coverage report using uv
	$(UV) run pytest $(TEST_DIR) --cov=$(SRC_DIR) --cov-report=xml -v
...
ci: ## Run continuous integration checks using uv
	...
	$(UV) run flake8 $(SRC_DIR) $(TEST_DIR)
	$(UV) run pylint $(SRC_DIR) $(TEST_DIR)
	$(UV) run mypy -p exact_online_sdk
